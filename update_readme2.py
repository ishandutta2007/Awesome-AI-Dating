...
def run_git(commit_msg):
    cwd_path = r"C:\\Users\\ishan\\Documents\\Projects\\Awesome-AI-Dating"
    subprocess.run(["git", "add", "."], cwd=cwd_path, check=False)
    subprocess.run(["git", "commit", "-m", commit_msg], cwd=cwd_path, check=False)
    subprocess.run(["git", "push"], cwd=cwd_path, check=False)

def main():
    readme_path = r"C:\\Users\\ishan\\Documents\\Projects\\Awesome-AI-Dating\\README.md"
    assets_dir = r"C:\\Users\\ishan\\Documents\\Projects\\Awesome-AI-Dating\\assets"
  ...
  <circle cx="90%" cy="50%" r="20" fill="rgba(255,255,255,0.2)">
    ...
    if "![Banner]" not in content:
        content = content.replace('# Awesome-AI-Dating', '# Awesome-AI-Dating\\n\\n![Banner](assets/banner.svg)')
        with open(readme_path, "w", encoding="utf-8") as f:
            f.write(content)
        run_git("added banner")

    # Step 4: Emojis
    if '## 💘 Top AI Dating' not in content:
    ...
    if 'alt="Discord"' not in content:
        ...
        content = content.replace('![Banner](assets/banner.svg)', '![Banner](assets/banner.svg)\\n\\n<div align="center">\\n' + left_badges + '\\n</div>')
        with open(readme_path, "w", encoding="utf-8") as f:
            f.write(content)
    ...
    if 'alt="GitHub followers"' not in content:
        ...
        content = content.replace('</div>', right_badge + '\\n</div>', 1)
        with open(readme_path, "w", encoding="utf-8") as f:
            f.write(content)
